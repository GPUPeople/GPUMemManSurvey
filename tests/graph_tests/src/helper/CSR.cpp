...
void storeCSRStandardFormat(const CSR<T>& mat, const char * file)
{
    ...
    {
        auto offset = mat.row_offsets[i];
        auto neighbours = mat.row_offsets[i + 1] - offset;
        for(auto j = offset; j < mat.row_offsets[i + 1]; ++j)
            fstream << mat.col_ids[j] << " ";
        fstream << "\n";
    }
}
...
template void storeCSRStandardFormat(const CSR<float>& mat, const char * file);
template void storeCSRStandardFormat(const CSR<double>& mat, const char * file);
