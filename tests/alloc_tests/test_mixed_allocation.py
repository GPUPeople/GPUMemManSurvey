		...
		for name, path in testcases.items():
			...
			while allocation_size <= largest_allocation_size:
				...
				with open(csv_path_free, "a", newline='') as csv_file:
					csv_file.write("\n" + str(smallest_allocation_size) + "-" + str(allocation_size) + ",")
				run_config = str(num_allocations) + " " + str(smallest_allocation_size) + " " + str(allocation_size) + " " + str(num_iterations) + " " + str(measure_on_device) + " " + str(test_warp_based) + " 1 " + str(free_memory) + " " + csv_path_alloc + " " + csv_path_free + " " + str(args.allocsize) + " " + str(args.device)
